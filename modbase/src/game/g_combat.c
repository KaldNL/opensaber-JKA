...
void body_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath ) {
	...
	// kaldor - Fuck Star Wars, this is JKA.
  qboolean doDisint = qfalse; //default is false
	...
	if (self->health < (GIB_HEALTH+1))
	{
		...
		if (self->client && (level.time - self->client->respawnTime) < 500) //kaldor - reducing from 2000
		{
			doDisint = qfalse; //kaldor - changing to True
		}
		else
		{
			doDisint = qtrue;
	...
	if (doDisint)
	{
		...
		else
		{
			...
			self->think = BodyRid;
			self->nextthink = level.time + 5; //kaldor - time before you can remove corpse
		}
		return;
	}
}
...
extern void RunEmplacedWeapon( gentity_t *ent, usercmd_t **ucmd );
	...
	*/
	...
	//kaldor - uncommented for gibs! ...recommented. 
...
//		self->client->ps.pm_flags |= PMF_UPDATE_ANIM;		// Make sure the pmove sets up the GHOUL2 anims.
		...
		self->takedamage = qfalse; //kaldor - again, fuck star wars. We're changing to qfalse.

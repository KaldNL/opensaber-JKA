...
void ClientCommand( int clientNum ) {
	...
	if (Q_stricmp (cmd, "uname") == 0) {
		trap_SendServerCommand( ent-g_entities, va("print \"opensaber version 12.03 (Alpha 1). Compiled with GCC 4.6.2 on Ubuntu 12.04 LTS. Date: February 12, 2012\n\""));
		return;
	}
	...
	if (Q_stricmp (cmd, "date") == 0) {
	  ...
	  strcat(str, "\"");
	  trap_SendServerCommand( ent-g_entities, va(str));
	  return;
	}
	//end clock
	
	
	//Kaldor - Display the license because I'm a freetard.
        if (Q_stricmp (cmd, "readlicense") == 0) {
       	        trap_SendServerCommand( ent-g_entities, va("print \"opensaber is licensed under the GNU GPL Version 2.\n\""));
                trap_SendServerCommand( ent-g_entities, va("print \"\n--GNU General Public License Version 2--\nThis program is free software; you can redistribute it and/or\nmodify it under the terms of the GNU General Public License\nas published by the Free Software Foundation; either version 2\nof the License, or (at your option) any later version.\n\nThis program is distributed in the hope that it will be useful,\nbut WITHOUT ANY WARRANTY; without even the implied warranty of\nMERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the\nGNU General Public License for more details.\nVisit www.gnu.org for more info.\n\""));
	return;
        }

	//kaldor - contact information
	if (Q_stricmp (cmd, "contactinfo") == 0) {
		trap_SendServerCommand( ent-g_entities, va("print \"^5IRC: Join #terminative3 on irc.quakenet.org and speak with Kaldor or Hugo.\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"^5Web: Visit terminative3.netgoo.org\n\""));
	return;
	}
...
trap_SendServerCommand( ent-g_entities, va("print \"^5out.\n\n\""));	
	...
	if (Q_stricmp (cmd, "manual") == 0) {
		trap_SendServerCommand( ent-g_entities, va("print \"^5--Client Commands--\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"manual: List available commands\n\""));
		trap_SendServerCommand( ent-g_entities, va("print \"uname: Display server's version of opensaber\n\""));

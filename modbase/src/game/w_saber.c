...
static GAME_INLINE int G_SaberAttackPower(gentity_t *ent, qboolean attacking)
{
	...
	{ //the attacker gets a boost to help penetrate defense.
			...
			ent->client->olderIsValid)
		{
			...
			{
			//kaldor - altering these to test parry effects
			case SS_STRONG:
				toleranceAmt = 20;
				break;
			case SS_MEDIUM:
...
#define SABER_HITDAMAGE 35 //interest point
	...
	else
	{
		...
		*/
		...
		case BOTH_A6_SABERPROTECT:
		  if ( animTimer < 450 ) //default < 650
			{//end of anim
			  return FORCE_LEVEL_0; //kaldor - altering for testing. Default 0.
			}
...
/*
		...
		if (SS_DUAL) {return FORCE_LEVEL_0;}

		else if (SS_STAFF) {return FORCE_LEVEL_0;} 
			
		else if (SS_STRONG) {return FORCE_LEVEL_3;}
			
		else if (SS_MEDIUM) {return FORCE_LEVEL_3;} 
		
        else if (SS_FAST) {return FORCE_LEVEL_3;}
        
        else {return FORCE_LEVEL_3;}
...
static GAME_INLINE qboolean CheckSaberDamage(gentity_t *self, int rSaberNum, int rBladeNum, vec3_t saberStart, vec3_t saberEnd, qboolean doInterpolate, int trMask, qboolean extrapolate )
{
	...
	saberBoxSize += (self->client->saber[rSaberNum].blade[rBladeNum].radius*1.0f); //kaldor - changed radius from 0.5f
		...
		if ( d_saberSPStyleDamage.integer )
		{
			...
			*/
			...
			{//the longer the trace, the more damage it does
				...
				if ( tr.fraction >= 0.5f ) //kaldor - changed from 1.0f
				{//allsolid?
					dmg = ceil( fDmg*traceLength*0.1f*0.33f );
				}
				else
				// kaldor - might be useful soon
				...
				self->client->ps.fd.saberAnimLevel == SS_DUAL)
			{
				if (saberInSpecial)
				{
					...
					{ //these attacks are long and have the potential to hit a lot so they will do less damage.
						dmg = 4; //kaldor - reducing because it's stupid. default 10.
					}
					else
					{
	...
	//kaldor - reference to ghosting, maybe?
